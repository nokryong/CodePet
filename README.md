...
- **캐릭터 이모티콘**: 에이전트가 답변마다 상황에 맞는 캐릭터 이모티콘을 1개 골라 붙입니다. 프롬프트에 이모티콘 의미 사전이 함께 전달되고, 답변 속 `[[CODEPET_EMOTE:키]]` 표기를 앱이 이미지로 바꿔 보여 줍니다(`src/chat-icon/emoticons`의 매니페스트 기반, 메시지당 최대 1개). 코드 블록·인라인 코드 안의 표기는 무시합니다.
